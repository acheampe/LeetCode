...
class Solution:
    def solve(self, board: list[list[str]]) -> None:
        ...
        def validRegion(r, c, currNeighbors, edgeRegion):
            
            if r < 0 or r >=m or c < 0 or c >= n and not board[r][c] == "0" and (r, c) not in isEdgeRegion:
                return 
            
            if not edgeRegion and r == 0 or r == (m - 1) or c == 0 or c == (n - 1):
                edgeRegion = True

            currNeighbors.append((r, c))
            for dr, dc in directions:
                nr, nc = r + dr, c + dc
                validRegion(nr, nc, currNeighbors, edgeRegion)
                    
            
        for i in range(m):
            for j in range(n):
                
                if (i, j) not in isEdgeRegion and board[i][j] == "O":
                    currRegion, falseRegion = [], False
                    validRegion(i, j, currRegion, falseRegion)
                    if not falseRegion:
                        for cr, cc in  currRegion:
                            isEdgeRegion.add((cr, cc))
