...
class Solution:
    def solve(self, board: list[list[str]]) -> None:
        ...
        def edgeRegion(r, c, currNeighbors):
            
            if r < 0 or r >=m or c < 0 or c >= n or board[r][c] == "X" or (r, c) in seen:
                return False
            
            touches_border = (r == 0 or r == (m - 1) or c == 0 or c == (n - 1))

            currNeighbors.append((r, c))
            seen.add((r,c))
            for dr, dc in directions:
                nr, nc = r + dr, c + dc
                if edgeRegion(nr, nc, currNeighbors):
                    touches_border = True
        ...
        for i in range(m):
            for j in range(n):
                ...
                if (i, j) not in seen and board[i][j] == "O":
                    currRegion = []
                    if not edgeRegion(i, j, currRegion):
